...
Create routes under `app/routes/app/` (already protected by middleware). Use dot notation for path segments — not nested folders.

For a resource called "notes", create these files:

- `app/routes/app/notes.tsx` — list page (`/app/notes`)
- `app/routes/app/notes.new.tsx` — create page (`/app/notes/new`)
- `app/routes/app/notes.$id.tsx` — detail/edit page (`/app/notes/:id`)

### List page: `app/routes/app/notes.tsx`
...
### Detail/edit page: `app/routes/app/notes.$id.tsx`
...
- Use dot-notation route files like `notes.new.tsx`, not nested `index.tsx` folders.
